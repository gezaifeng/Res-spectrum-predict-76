...
def visualize_prediction(true_spectrum, predicted_spectrum, wavelengths=None):
    ...
    if wavelengths is None:
        wavelengths = np.arange(1, 77)  # 默认波长点 1~77
...
# 训练数据路径（用于获取数据均值 & 标准差）
train_dir = "D:\\Desktop\\model\\Res spectrum predict\\dataset-76\\train"

# 2. 加载训练数据集（确保光谱数据归一化）
train_dataset = SpectralDataset(train_dir, normalize_spectra=True)

# 3. 预测单个样本
test_rgb_path = "D:\\Desktop\\model\\Res spectrum predict\\dataset-76\\val\\rgb_0292.npy"
predicted_spectrum = predict(test_rgb_path, train_dataset)  # 传入 train_dataset

print(f"预测的光谱数据: {predicted_spectrum}")

# 可视化单个预测
true_spectrum = np.load("D:\\Desktop\\model\\Res spectrum predict\\dataset-76\\val\\spectral_0292.npy")
